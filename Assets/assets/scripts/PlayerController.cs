...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // crouching logic
        // Note: We check if grounded OR if we are already crouching (to prevent standing up into a ceiling)
        ...
        {
            // If we release S or start walking, stand up
            if (isCrouching) StopCrouch();
        }

        // slideinput
        if (Input.GetKeyDown(KeyCode.S) && isGrounded && moveInput != 0 && canSlide)
        {
    ...
    // Crouch function  seperate form the slide function
    void StartCrouch()
    {
        isCrouching = true;
